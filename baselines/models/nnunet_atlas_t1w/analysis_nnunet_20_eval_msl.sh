#!/bin/bash
#SBATCH --job-name=nnunet_eval_msl
...
# MSL hypothesis test: on the fold-0 cross-validation split, merge the MSL
# (size-class) predictions back to binary and compare Dice + size-binned lesion
# recall head-to-head against the binary baseline fold-0 predictions. Both use
# the same images/splits, so this isolates the MSL relabeling effect. The win
# condition is higher small-lesion recall without losing Dice.
#
#   sbatch baselines/models/nnunet_atlas_t1w/analysis_nnunet_20_eval_msl.sh
...
def size_binned_recall(pred_bin, gt_bin):
    """Pooled detected/total GT components per size bin (>=1 voxel overlap)."""
...
def evaluate(val_dir, merge_multiclass):
    ...
    for cid in VAL:
        ...
        for b in BINS:
            det_all[b[0]] += det[b[0]]; tot_all[b[0]] += tot[b[0]]
        used += 1
...
out = {}
if MSL_VAL.is_dir() and any(MSL_VAL.glob("*.nii.gz")):
    out["msl"] = evaluate(MSL_VAL, merge_multiclass=True)
else:
...
lines += [
    f"Cases: binary n={b['n']}" + (f", MSL n={m.get('n')}" if "n" in m else f" (MSL: {m.get('error')})"),
    "",
...
if "mean_dice" in m and m["mean_dice"] is not None:
    lines.append(f"| MSL (merged) | {m['mean_dice']:.4f} | {m['median_dice']:.4f} |")
