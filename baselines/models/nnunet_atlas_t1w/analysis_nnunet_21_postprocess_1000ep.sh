#!/bin/bash
#SBATCH --job-name=nnunet_postproc_1000ep
...
cd "$REPO"
mkdir -p "$REPO/logs" "$REPO/baselines/reports/nnunet_1000ep_crossval"
...
RESULT_DIR="$nnUNet_results/Dataset502_ATLASR30/nnUNetTrainer__nnUNetPlans__3d_fullres"
...
cp -f "$nnUNet_results/Dataset502_ATLASR30"/inference_instructions.txt \
      "$REPO/baselines/reports/nnunet_1000ep_crossval/" 2>/dev/null || true
cp -f "$CV_DIR/summary.json" \
      "$REPO/baselines/reports/nnunet_1000ep_crossval/crossval_summary.json" 2>/dev/null || true
cp -f "$CV_DIR/postprocessed/summary.json" \
      "$REPO/baselines/reports/nnunet_1000ep_crossval/crossval_postprocessed_summary.json" 2>/dev/null || true
