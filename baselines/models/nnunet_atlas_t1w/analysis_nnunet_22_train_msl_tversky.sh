#!/bin/bash
#SBATCH --job-name=nnunet_train_msl_tv
...
# classes) with a PRECISION-AWARE Tversky(0.7/0.3)+CE loss, to keep MSL's
# small-lesion recall gain while recovering the ~0.8% Dice it cost (the extra
# small-class predictions are mostly false positives; Tversky penalizes FP).
# Fold 0 first as a cheap hypothesis test: sbatch --array=0 ...
# Merge size classes back to binary at inference (step 23). Preempt-safe.
#
#   sbatch --array=0 baselines/models/nnunet_atlas_t1w/analysis_nnunet_22_train_msl_tversky.sh
...
# Also install the precision-aware Tversky+CE trainer used by this run.
python "$REPO/baselines/models/nnunet_atlas_t1w/nnunet_helpers/install_trainer_variant.py" \
  --source "$REPO/baselines/models/nnunet_atlas_t1w/nnunet_helpers/nnUNetTrainerTverskyCELoss.py"
python - <<'PY'
import torch
...
TRAIN_ARGS=(nnUNetv2_train 503 3d_fullres "$FOLD" -tr nnUNetTrainerTverskyCELoss_250epochs)
mapfile -t EXISTING < <(find "$nnUNet_results/Dataset503_ATLASR30_MSL" -path "*nnUNetTrainerTverskyCELoss_250epochs*3d_fullres/fold_${FOLD}/checkpoint_*.pth" -type f 2>/dev/null | sort)
if (( ${#EXISTING[@]} > 0 )); then
  TRAIN_ARGS+=(--c)
  echo "[info] fold $FOLD: existing checkpoint found; continuing with --c"
else
  echo "[info] fold $FOLD: fresh 250-epoch MSL+Tversky run"
fi
