#!/bin/bash
#SBATCH --job-name=nnunet_train_topk10
...
# Train nnU-Net v2 3d_fullres on Dataset502_ATLASR30 (ATLAS R3.0: 1450 cases,
# fresh stratified 5-fold) with the built-in Dice+TopK10 loss (1000 epochs) --
# MAPPING's winning ISLES recipe. TopK10 mines the hardest 10% of CE voxels,
# typically improving boundaries / hard regions. Compare consolidated CV Dice to
# the plain 1000-epoch model's 0.6528. One fold per array task. Preempt-safe.
#
#   sbatch baselines/models/nnunet_atlas_t1w/analysis_nnunet_24_train_topk10.sh
...
TRAIN_ARGS=(nnUNetv2_train 502 3d_fullres "$FOLD" -tr nnUNetTrainerDiceTopK10Loss)
mapfile -t EXISTING < <(find "$nnUNet_results/Dataset502_ATLASR30" -path "*nnUNetTrainerDiceTopK10Loss__nnUNetPlans__3d_fullres/fold_${FOLD}/checkpoint_*.pth" -type f 2>/dev/null | sort)
if (( ${#EXISTING[@]} > 0 )); then
  TRAIN_ARGS+=(--c)
  echo "[info] fold $FOLD: existing checkpoint found; continuing with --c"
else
  echo "[info] fold $FOLD: fresh 1000-epoch Dice+TopK10 run"
fi
...
echo "[done] Dataset502_ATLASR30 fold $FOLD training complete"
