#!/bin/bash
#SBATCH --job-name=nnunet_predict_r30
...
# Skull-strip soop_bench T1w with SynthStrip, run the ATLAS R3.0 nnU-Net
# 5-fold ensemble (Dataset502, with TTA), rigidly warp the T1w-space
# prediction into TRACE/DWI space, and write one prediction per subject under
# work/predictions_nnunet_r30/.
#
# NOTE: R3.0 training INCLUDED the 169 sub-soop* subjects, so soop_bench
# (sub-1..14) may overlap with training -- this number can be optimistically
# biased. Treated as the OOD proxy with that caveat (see project memory).
...
PREDICTION_VERSION="r30-5fold-tta"
...
PREDS="$REPO/work/predictions_nnunet_r30"
TMP_ROOT="$REPO/work/predictions_nnunet_r30_tmp"
...
RESULT_FOLD="$nnUNet_results/Dataset502_ATLASR30/nnUNetTrainer_250epochs__nnUNetPlans__3d_fullres/fold_0"
