#!/bin/bash
#SBATCH --job-name=nnunet_train_1000ep
...
# 1284 R-site + 169 SOOP, fresh stratified 5-fold) with the FULL default
# 1000-epoch schedule (default nnUNetTrainer) to test whether a longer schedule
# beats the 250-epoch CV Dice of 0.6372. One fold per array task. Preempt-safe
# (USR1/TERM -> requeue); resumes with --c.
#
#   sbatch baselines/models/nnunet_atlas_t1w/analysis_nnunet_15_train_1000ep.sh
...
TRAIN_ARGS=(nnUNetv2_train 502 3d_fullres "$FOLD")
mapfile -t EXISTING < <(find "$nnUNet_results/Dataset502_ATLASR30" -path "*nnUNetTrainer__nnUNetPlans__3d_fullres/fold_${FOLD}/checkpoint_*.pth" -type f 2>/dev/null | sort)
if (( ${#EXISTING[@]} > 0 )); then
  TRAIN_ARGS+=(--c)
  echo "[info] fold $FOLD: existing checkpoint found; continuing with --c"
else
  echo "[info] fold $FOLD: fresh 1000-epoch run (default nnUNetTrainer)"
fi
