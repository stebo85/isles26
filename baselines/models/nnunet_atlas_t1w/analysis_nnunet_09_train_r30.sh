#!/bin/bash
#SBATCH --job-name=nnunet_train_r30
...
# 1284 R-site + 169 SOOP, fresh stratified 5-fold). One fold per array task,
# 250-epoch schedule. Preempt-safe (USR1/TERM -> requeue); resumes with --c.
#
...
TRAIN_ARGS=(nnUNetv2_train 502 3d_fullres "$FOLD" -tr nnUNetTrainer_250epochs)
mapfile -t EXISTING < <(find "$nnUNet_results/Dataset502_ATLASR30" -path "*nnUNetTrainer_250epochs*3d_fullres/fold_${FOLD}/checkpoint_*.pth" -type f 2>/dev/null | sort)
if (( ${#EXISTING[@]} > 0 )); then
  TRAIN_ARGS+=(--c)
  echo "[info] fold $FOLD: existing checkpoint found; continuing with --c"
else
  echo "[info] fold $FOLD: fresh 250-epoch run"
fi
